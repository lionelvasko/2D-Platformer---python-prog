...
from sprites import Sprite, MovingSprite, AnimatedSprite, Spike, ParticleEffectSprite
from player import Player
from groups import AllSprites
from enemies import Tooth, Canon, Ball
...
class Level:
    def __init__(self, tmx_map, level_frames, audio_files, data, switch_stage):
        ...
        # level to unlock in the overworld
        ...
        # frames
        self.canonball_surf = level_frames['pearl']
        self.particle_frames = level_frames['particle']

        # audio
        self.damage_sound = audio_files['damage']
        self.damage_sound.set_volume(0.5)
        self.canonball_sound = audio_files['pearl']
    ...
    def setup(self, tmx_map, level_frames, audio_files):
        ...
        for obj in tmx_map.get_layer_by_name('Enemies'):
            ...
            if obj.name == 'canon':
                Canon(
                    pos=(obj.x, obj.y),
                    frames=level_frames['canon'],
                    groups=(self.all_sprites, self.collision_sprites),
                    reverse=obj.properties['reverse'],
                    player=self.player,
                    create_canonball=self.create_canonball)
    ...
    # create canonball
    def create_canonball(self, pos, direction):
        Ball(pos, (self.all_sprites, self.damage_sprites, self.canonball_sprites), self.canonball_surf, direction, 150)
        self.canonball_sound.play()

    # collision detection checks with canonballs and sprites
    def canonball_collision(self):
        for sprite in self.collision_sprites:
            sprite = pygame.sprite.spritecollide(sprite, self.canonball_sprites, True)
            if sprite:
                ParticleEffectSprite(sprite[0].rect.center, self.particle_frames, self.all_sprites)

    # collision detection checks with damage sprites (canonball) and player
    ...
    # run the level and update the sprites
    def run(self, dt):
        self.display_surface.fill('black')

        self.all_sprites.update(dt)
        self.canonball_collision()
        self.hit_collision()
