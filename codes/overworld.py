...
class Overworld:
    ...
    def setup(self, tmx_map, overworld_frames):
        ...
        for obj in tmx_map.get_layer_by_name('Objects'):
            if obj.name == 'palm':
                AnimatedSprite((obj.x, obj.y), overworld_frames['palms'],
                               self.all_sprites, Z_LAYERS['main'], randint(4, 6))
            else:
        ...
        for obj in tmx_map.get_layer_by_name('Nodes'):
            ...
            if obj.name == 'Node' and obj.properties['stage'] == self.data.current_level:
                self.icon = Icon((obj.x + TILE_SIZE / 2, obj.y + TILE_SIZE / 2),
                                 self.all_sprites, overworld_frames['icon'])
        ...
        for key, path in path_tiles.items():
            for index, tile in enumerate(path):
                if 0 < index < len(path) - 1:
                    ...
                    # vertical, horizontal or corner path
    ...
    # run overworld and update sprites
    def run(self, dt):
        self.input()
