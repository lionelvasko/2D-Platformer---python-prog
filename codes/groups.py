...
class WorldSprites(pygame.sprite.Group):
    """Group for the sprites in the overworld."""

    def __init__(self, data):
        """Initialize the WorldSprites object with data."""
        super().__init__()
    ...
    def draw(self, target_pos):
        """Draw the sprites in the overworld with the target position (Camera)."""
        self.offset.x = -(target_pos[0] - WINDOW_WIDTH / 2)
        self.offset.y = -(target_pos[1] - WINDOW_HEIGHT / 2)

        # background
        for sprite in sorted(self, key=lambda sprite_inner: sprite_inner.z):
            if sprite.z < Z_LAYERS['main']:
                if sprite.z == Z_LAYERS['path']:
                    ...
                    self.display_surface.blit(sprite.image, sprite.rect.topleft + self.offset)
        # main
        for sprite in sorted(self, key=lambda sprite_inner: sprite_inner.rect.centery):
            if sprite.z == Z_LAYERS['main']:
...
class AllSprites(pygame.sprite.Group):
    """Group for the sprites in the level."""

    def __init__(self, width, height, clouds, horizon_line, bg_tile=None, top_limit=0):
        """Initialize the AllSprites object with width, height, clouds, horizon_line, bg_tile, and top_limit."""
        super().__init__()
        self.display_surface = pygame.display.get_surface()
        self.offset = vector()
    ...
    def draw(self, target_pos, dt):
        """Draw the sprites in the level with the target position (Camera)."""
        self.offset.x = -(target_pos[0] - WINDOW_WIDTH / 2)
        self.offset.y = -(target_pos[1] - WINDOW_HEIGHT / 2)
        ...
        # sprites
        for sprite in sorted(self, key=lambda spirte_inner: spirte_inner.z):
